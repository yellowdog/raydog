from yellowdog_client.model import Task, TaskOutput, TaskSearch, TaskStatus
...
class RayDogHead(RayDog):
    def __init__(self):
        ...
        self.workreq = self.ydworkapi.get_work_requirement_by_name(
            self.namespace, self._get_wr_name()
        )
    ...
    def add_workers(self, howmany=1):
        ...
        newtasks = self.ydworkapi.add_tasks_to_task_group(
            self._get_worker_task_group(), tasklist
        )
